...
def get_next_word_packet(s):
    ...
    while x < 10:
        ...
        #print(len(packet_buffer[2:2+length]))
        #print(len(packet_buffer[2+length:]))
        if len(packet_buffer[2:2+length]) >= length:
            encoded_word = packet_buffer[2:2+length]
            #print(f"hello {encoded_word}")
